...
export function AICoachTab({ studentId, studentName, weekNumber, onSendToAskAI }: AICoachTabProps) {
  ...
  // Keep activeSpeaker in a ref so the speech recognition closure always reads the latest value
  const activeSpeakerRef = useRef(activeSpeaker);
  activeSpeakerRef.current = activeSpeaker;

  // Speaker options
  const speakers = ["Coach", studentName];
  ...
  // Lazily create a session only when there's content to save
  async function ensureSession(): Promise<string> {
    if (sessionId) return sessionId;
    const session = await createCoachSession(studentId, sessionType, weekNumber, destination);
  ...
  function startRecording() {
    ...
    setPublished(false);
    setSaved(false);
    setPublishTitle(`Week ${weekNumber} ${sessionType} — ${studentName}`);

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const SpeechRecognition = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition;

    if (!SpeechRecognition) {
      // Fallback: manual textarea — no DB call until user types and stops
      setIsRecording(true);
      return;
    }
    ...
    // Browser auto-stops after silence — restart to keep continuous
    recognition.onend = () => {
      if (recognitionRef.current) {
        try { recognitionRef.current.start(); } catch { /* ignore restart errors */ }
      }
    };

    recognition.onerror = (e: { error: string }) => {
      // not-allowed = mic permission denied — stop completely
      if (e.error === "not-allowed" || e.error === "service-not-allowed") {
        recognitionRef.current = null;
        setIsRecording(false);
      }
      // other errors (network, aborted) — onend will restart
    };
  ...
  async function stopRecording() {
    ...
    recognitionRef.current = null;
    setIsRecording(false);
    // Don't auto-save — user will choose Save or Discard
  }
    ...
    <div className="space-y-8">
      ...
      <div className="bg-card border border-border rounded-xl p-5 space-y-5">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-bold">Record New Session</h3>
          <span className="ml-auto text-xs text-muted-foreground">Week {weekNumber}</span>
        </div>
        ...
        {/* Speaker tag buttons (visible when recording or after recording with transcript) */}
        ...
        <div className="flex gap-3 items-center">
          ...
          {isRecording && (
            <span className="text-xs text-muted-foreground">
              Recording as <strong>{activeSpeaker}</strong>…
            </span>
          )}
        </div>
        ...
        {summary && (
          <div className="space-y-4 pt-2">
            ...
            <h4 className="text-sm font-bold text-foreground">AI Analysis</h4>
            ...
            {destination === "student" && (
              <>
                ...
                <div className="flex gap-3 flex-wrap text-sm">
                  {summary.sentiment && (
                    <span className={`px-2.5 py-1 rounded-full bg-muted font-medium ${sentimentColor}`}>
                      Sentiment: {summary.sentiment}
                    </span>
                  )}
                  {summary.studentEngagement && (
                    <span className={`px-2.5 py-1 rounded-full bg-muted font-medium ${engagementColor}`}>
                      Engagement: {summary.studentEngagement}
                    </span>
                  )}
                  {summary.coachingTone && (
                    <span className="px-2.5 py-1 rounded-full bg-muted font-medium text-muted-foreground">
                      Tone: {summary.coachingTone}
                    </span>
                  )}
                </div>
              </>
            ...
            <div className="flex flex-col gap-3 pt-1">
              ...
              {destination === "student" && !published && (
                <div className="bg-muted/30 border border-border rounded-lg p-4 space-y-3">
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Publish to {studentName}</p>
                  <input
                    value={publishTitle}
          ...
          {(["student", "general"] as SessionTab[]).map((t) => (
            <button
              ...
              className={`pb-2 text-sm font-semibold border-b-2 transition-colors ${
                sessionTab === t ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"
              }`}
            >
              {t === "student" ? `For ${studentName}` : "General Knowledge"}
            </button>
          ))}
        </div>
