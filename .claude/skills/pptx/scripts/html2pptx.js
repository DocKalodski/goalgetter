 ...
 *     autofix: true  // Enable auto-fix for overflow and styling issues
 *   });
 ...
 *
 * OPTIONS:
 *   - autofix: boolean (default: false) - Auto-fix HTML overflow and styling issues
 *   - autofixOptions: { maxIterations: number, dryRun: boolean } - Options for auto-fix
 *   - tmpDir: string - Temporary directory for processing
 *   - slide: Slide - Existing slide to add content to
 *
 * VALIDATION:
 ...
 *   {
 *     slide: Slide,
 *     placeholders: Array<{ id, x, y, w, h }>,
 *     fixReport: { inputFile, outputFile, success, fixes, warnings } | null
 *   }
 */

const path = require('path');

// Resolve modules from skill's node_modules directory
const skillDir = path.resolve(__dirname, '..');
const skillModules = path.join(skillDir, 'node_modules');

function skillRequire(moduleName) {
    return require(path.join(skillModules, moduleName));
}
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  let processFile = htmlFile;
  let fixReport = null;

  // Auto-fix HTML issues if enabled
  if (autofix) {
    const htmlAutofix = require('./html-autofix');
    const result = await htmlAutofix(htmlFile, autofixOptions);
    if (result.fixedPath) {
      processFile = result.fixedPath;
      fixReport = result.report;
    } else if (result.report && !result.report.success) {
      // Auto-fix failed, continue with original file but note the warning
      fixReport = result.report;
    }
  }
  ...
  try {
    ...
    const filePath = path.isAbsolute(processFile) ? processFile : path.join(process.cwd(), processFile);
    ...
    return { slide: targetSlide, placeholders: slideData.placeholders, fixReport };
  } catch (error) {
