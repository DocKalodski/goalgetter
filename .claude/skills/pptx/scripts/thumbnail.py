...
Examples:
    python thumbnail.py idea-process-to-agent-factory-b0c4093b.pptx
    # Creates: idea-process-to-agent-factory-b0c4093b-thumbnails.jpg (session-specific)
    # Outputs:
    #   Created 1 grid(s):
    #     - idea-process-to-agent-factory-b0c4093b-thumbnails.jpg

    python thumbnail.py large-deck.pptx custom-prefix --cols 4
    # Creates: custom-prefix-1.jpg, custom-prefix-2.jpg, etc. (custom prefix)
    # Outputs:
    #   Created 3 grid(s):
    #     - custom-prefix-1.jpg
    #     - custom-prefix-2.jpg
...
# Tool paths - check common locations
def find_soffice():
    ...
    for path in candidates:
        if os.path.isfile(path) or path == "soffice":
            return path
    return "soffice"
...
def find_pdftoppm():
    ...
    candidates = [
        r"C:\Users\Public\Tools\poppler\poppler-25.12.0\Library\bin\pdftoppm.exe",
        r"C:\ProgramData\chocolatey\bin\pdftoppm.exe",
        "pdftoppm",  # fallback to PATH
    ]
    for path in candidates:
        if os.path.isfile(path) or path == "pdftoppm":
            return path
    return "pdftoppm"
...
def get_session_thumbnail_path(pptx_path):
    """Generate session-specific thumbnail path from PPTX filename.

    When no output prefix is provided, derive the thumbnail name from the input file.
    This preserves thumbnails per ideation session instead of overwriting.

    Input: idea-process-to-agent-factory-b0c4093b.pptx
    Output: idea-process-to-agent-factory-b0c4093b-thumbnails (without extension)
    """
    stem = Path(pptx_path).stem
    return str(Path(pptx_path).parent / f"{stem}-thumbnails")
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="Output prefix for image files (default: derives from input filename, e.g., idea-foo-abc123-thumbnails.jpg)",
    )
    parser.add_argument(
        "--cols",
        type=int,
    ...
    # If no output prefix provided, derive from input filename for session-specific naming
    if args.output_prefix is None:
        output_prefix = get_session_thumbnail_path(input_path)
    else:
...
def convert_to_images(pptx_path, temp_dir, dpi):
    ...
    result = subprocess.run(
        [PDFTOPPM_PATH, "-jpeg", "-r", str(dpi), str(pdf_path), str(temp_dir / "slide")],
        capture_output=True,
        text=True,
    )
