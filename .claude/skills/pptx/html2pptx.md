...
- `options` (object, optional):
  ...
  - `autofix` (boolean): Auto-fix HTML overflow and styling issues (default: false)
  - `autofixOptions` (object): Options for auto-fix { maxIterations: 5, dryRun: false }
...
```javascript
{
    ...
    fixReport: {                    // Auto-fix report (null if autofix disabled)
        inputFile: string,
...
Enable automatic fixing of common HTML issues before PPTX conversion. This feature addresses overflow and styling validation errors without manual intervention.

### Enabling Auto-Fix

```javascript
const { slide, placeholders, fixReport } = await html2pptx('slide.html', pptx, {
    autofix: true
});

// Check what was fixed
if (fixReport && fixReport.fixes.length > 0) {
    console.log(`Fixed ${fixReport.fixes.length} issues:`);
...
| Issue | Fix Strategy |
|-------|--------------|
| Background on text elements (`<p>`, `<h1>`-`<h6>`) | Wraps text in `<div>`, moves background to wrapper |
| Border on text elements | Wraps text in `<div>`, moves border to wrapper |
| Box-shadow on text elements | Wraps text in `<div>`, moves shadow to wrapper |
| Content overflow (scrollHeight > height) | Iteratively reduces padding/margins, then font sizes |

### Auto-Fix Options

```javascript
const { slide, fixReport } = await html2pptx('slide.html', pptx, {
    autofix: true,
    autofixOptions: {
        maxIterations: 5,    // Max overflow fix iterations (default: 5)
        dryRun: false        // Report issues without fixing (default: false)
    }
});
```
...
```javascript
{
    inputFile: 'slide.html',           // Original file
    outputFile: 'slide-fixed.html',    // Fixed file (null if no fixes needed)
    success: true,                      // Whether auto-fix completed
    fixes: [                            // Array of fixes applied
        { type: 'styling', element: '<p>', fix: 'wrapped in div, moved background to wrapper' },
        { type: 'overflow', fix: 'reduced padding/margins by 10%', iterations: 2 }
    ],
    warnings: []                        // Any warnings (e.g., could not fully fix overflow)
}
```

### What Cannot Be Auto-Fixed

- **CSS gradients** - Must be pre-rendered as PNG images
- **Background images on DIV** - Not supported in PowerPoint
- **Custom fonts** - Must use web-safe fonts
- **Extreme overflow** - Content that exceeds bounds by more than ~25%
